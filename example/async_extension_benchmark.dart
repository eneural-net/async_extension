...
Future<Benchmark> doBenchmarkAwait(Computation computation, int iterations) async {
  ...
  var sum = (await Future.wait(results)).reduce((value, element) => value+element);

  var end = DateTime.now() ;
  var time = Duration( milliseconds: end.millisecondsSinceEpoch - init.millisecondsSinceEpoch  );

  return Benchmark('await', computation, sum, iterations, time);
}

/// Benchmark using a method optimized with `async_extension`:
FutureOr<Benchmark> doBenchmarkOptimized(Computation computation, int iterations) {
  var init = DateTime.now() ;
...
void main() async {
  ...
  for (var session = 0; session < 10; ++session) {
    var benchmarkAwaitSync = await doBenchmarkAwait( ComputationSync() , iterations );
    var benchmarkAwaitAsync = await doBenchmarkAwait( ComputationAsync() , iterations );

    var benchmarkOptimizedSync = await doBenchmarkOptimized( ComputationSync() , iterations );
    var benchmarkOptimizedAsync = await doBenchmarkOptimized( ComputationAsync() , iterations );
